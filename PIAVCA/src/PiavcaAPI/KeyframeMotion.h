 ...
 * License.
 *
 * The Original Code is KeyframeMotion.h.
 *
 ...
 */
	...
	class PIAVCA_DECL KeyframeMotion : public Motion 
	{
	protected:
	...
	public:
		...
		//KeyframeMotion(MotionImp *_imp):imp(_imp), facial (false) {};//{imp->inter = this;};
		 ...
		 */
		KeyframeMotion(tstring fileName, int flags = TRANS_NONE, Motion *basePosture = NULL) 
			: imp(0)
		{
		...
		KeyframeMotion(bool _facial = false): imp(0)
		{
			initMotion(_PSTR(""), _facial);
			//imp->inter = this;
		};
		
		KeyframeMotion(MotionImp *_imp);

		//! copy constructor
		KeyframeMotion(const KeyframeMotion &mot):Motion(mot), imp(0)
		{
			(*this) = mot;
		};
		//! assignment
		const KeyframeMotion &operator=(const KeyframeMotion &mot)
		{
			//facial = mot.facial;
			name = mot.name;
		 ...
		 *	and the values saved as keyframes;
		 */
		const KeyframeMotion &operator=(Motion &mot);
