...
{
       0,   158,   158,   163,   180,   183,   184,   187,   197,   201,
     202,   205,   232,   233,   234,   235,   238,   239,   240,   243,
     256,   288,   291,   294,   297,   298,   301
};
#endif
...
{
  "$end", "error", "$undefined", "HIERARCHY", "ROOT", "OFFSET",
  "CHANNELS", "JOINT", "END_SITE", "MOTION", "FRAMES", "FRAME_TIME",
  "NUMBER", "NUMBER_INT", "NAME", "DOF_NAME", "L_BRACK", "R_BRACK",
  "$accept", "bvh_file", "heirarchy_sec", "root_node", "joint_body",
  "offset_spec", "channel_spec", "channel_list", "channel", "joint_list",
  "end_site", "joint_spec", "joint_name", "motion_sec", "no_frames",
  "frame_time", "keyframe_list", "keyframe", 0
};
#endif
...
#line 163 "bvhParser.ypp"
  ...
  case 4:
#line 180 "bvhParser.ypp"
    {delete (yyvsp[(3) - (3)].intList);}
    break;

  case 5:
#line 183 "bvhParser.ypp"
    {(yyval.intList) = (yyvsp[(4) - (5)].intList);}
    break;

  case 6:
#line 184 "bvhParser.ypp"
    {(yyval.intList) = new vector<int>();}
    break;

  case 7:
#line 188 "bvhParser.ypp"
    {
     if(currentTrack >= 0)
       {
  ...
  case 8:
#line 198 "bvhParser.ypp"
    {currentChannel = 0;}
    break;

  case 9:
#line 201 "bvhParser.ypp"
    {}
    break;

  case 10:
#line 202 "bvhParser.ypp"
    {}
    break;

  case 11:
#line 205 "bvhParser.ypp"
    {
				  if(currentTrack >= 0)
				    {
				      if((yyvsp[(1) - (1)].intVal) >= 0)
						{
							tracks[currentTrack].channel[currentChannel] = (yyvsp[(1) - (1)].intVal);
							tracks[currentTrack].negateChannel[currentChannel] = false;
						}
						else
						{
							if((yyvsp[(1) - (1)].intVal) == -6)
				    ...
				    {
				      rootChannel[currentChannel] = (yyvsp[(1) - (1)].intVal);
				    }
				  currentChannel++;
  ...
  case 12:
#line 232 "bvhParser.ypp"
    {(yyval.intList) = (yyvsp[(1) - (2)].intList); for(unsigned int i = 0; i < (yyvsp[(2) - (2)].intList)->size(); i++) (yyval.intList)->push_back((*(yyvsp[(2) - (2)].intList))[i]); delete (yyvsp[(2) - (2)].intList);}
    break;

  case 13:
#line 233 "bvhParser.ypp"
    {(yyval.intList) = (yyvsp[(1) - (2)].intList);}
    break;

  case 14:
#line 234 "bvhParser.ypp"
    {(yyval.intList) = (yyvsp[(1) - (1)].intList);}
    break;

  case 15:
#line 235 "bvhParser.ypp"
    {(yyval.intList) = new vector<int>();}
    break;

  case 16:
#line 238 "bvhParser.ypp"
    {}
    break;

  case 17:
#line 239 "bvhParser.ypp"
    {}
    break;

  case 18:
#line 240 "bvhParser.ypp"
    {}
    break;

  case 19:
#line 244 "bvhParser.ypp"
    {
				       if(g_pBasePosture != NULL && !g_pBasePosture->isNull(tracks[(yyvsp[(1) - (2)].intVal)].trackId))
					 tracks[(yyvsp[(1) - (2)].intVal)].multiplier = g_pBasePosture->getQuatValueAtTime(tracks[(yyvsp[(1) - (2)].intVal)].trackId, 0.0).inverse();
				       (yyvsp[(2) - (2)].intList)->push_back((yyvsp[(1) - (2)].intVal));
				       //std::cout << $2->size();
				       //for (int i = 0; i < $2->size(); i)
						//	std:: cout << (*$2)[i] << std::endl;
				       //std::cout << std::endl;
				       (yyval.intList) = (yyvsp[(2) - (2)].intList);
                                     }
    break;

  case 20:
#line 256 "bvhParser.ypp"
    {	
  ...
  currentChannel = 0;
  tracks[currentTrack].name = StringToTString((yyvsp[(2) - (2)].name));
  ...
  case 21:
#line 288 "bvhParser.ypp"
    {}
    break;

  case 22:
#line 291 "bvhParser.ypp"
    {	g_noFrames = (yyvsp[(2) - (2)].intVal);}
    break;

  case 23:
#line 294 "bvhParser.ypp"
    {	frameTime = (yyvsp[(2) - (2)].floatVal); }
    break;

  case 24:
#line 297 "bvhParser.ypp"
    {}
    break;

  case 25:
#line 298 "bvhParser.ypp"
    {}
    break;

  case 26:
#line 302 "bvhParser.ypp"
    {
			if(currentTrack < 0)
			{	
		 ...
		 if(currentTrack < 0)
	      {
			if(rootChannel[currentChannel] > 2 && rootChannel[currentChannel] < 6)
			{
			    ...
			    if (g_bNoRootPosCorrection)
			    {
					...
					{
					case 0: 
						currentRootPos += Vec::XAxis()*(yyvsp[(1) - (1)].floatVal);
						break;
					case 1: 
						currentRootPos += Vec::YAxis()*(yyvsp[(1) - (1)].floatVal);
						break;
					case 2: 
						currentRootPos += Vec::ZAxis()*(yyvsp[(1) - (1)].floatVal);
						break;
					default:
						break;
					}
			    ...
			    {
					currentRootPos += g_Axes[rootChannel[currentChannel] - 3]*(yyvsp[(1) - (1)].floatVal);
				}
			}
			else
			{
			    //float angle = -Piavca::degToRad($1);
			    float angle = Piavca::degToRad((yyvsp[(1) - (1)].floatVal));
			    if(g_ReverseOrder)
		  ...
		  if(tracks[currentTrack].channel[currentChannel] <= 2)
			  ...
			  float angle = (yyvsp[(1) - (1)].floatVal);
			 ...
			 if(g_ReverseOrder)
			  ...
			  if(g_bTest)
			      tracks[currentTrack].currentEuler[tracks[currentTrack].channel[currentChannel]] = (yyvsp[(1) - (1)].floatVal);
		      }
	      }
		 currentChannel++;
		 fprintf(g_file, " %f ", (yyvsp[(1) - (1)].floatVal));
